...
public class CMISGateway {
	...
	public CMISGateway(String inUrl, String userName, String userPass, String inBinding, PluginCallback callback)
			throws Exception {
		...
		try {
			...
			throw new PluginOperationFailedException("Failed to establish a session with the cmis Repository ",
					(Throwable) e);
		}
	}
	...
	public Document getDocumentMetadata(String inUrl) throws IOException, PluginOperationFailedException {
		...
		return getDocument(inUrl, CMISUtils.CMIS_DIRECTORY.equals(object.getBaseTypeId().toString()), object);

	}

	private String getPathFromUrl(String inUrl) throws MalformedURLException, PluginOperationFailedException {
		String uri = inUrl.replaceAll(this.getBaseUrl(), CMISUtils.EMPTY_STRING).replaceAll("%20", " ");
		if (uri.isEmpty()) {
		...
		try {
			...
			for (CmisObject child : children) {
				...
				if (listContainers && CMISUtils.CMIS_DIRECTORY.equalsIgnoreCase(child.getBaseTypeId().toString())) {
					documentList.add(getDocument(url, listContainers, child));
				} else if (!listContainers && CMISUtils.CMIS_FILE.equalsIgnoreCase(child.getBaseTypeId().toString())) {
					documentList.add(getDocument(url, listContainers, child));
				} else if (!listContainers
						&& CMISUtils.CMIS_DIRECTORY.equalsIgnoreCase(child.getBaseTypeId().toString())) {
					documentList.add(getDocument(url, !listContainers, child));
				}
			}
	...
	private String getUrlFromObject(CmisObject child) throws MalformedURLException, PluginOperationFailedException {
		...
		if (CMISUtils.CMIS_FILE.equalsIgnoreCase(child.getBaseTypeId().toString())) {
		...
		if (CMISUtils.CMIS_ITEM.equalsIgnoreCase(child.getBaseTypeId().toString())
				|| CMISUtils.CMIS_RELATIONSHIP.equalsIgnoreCase(child.getBaseTypeId().toString())
				|| CMISUtils.CMIS_POLICY.equalsIgnoreCase(child.getBaseTypeId().toString())
				|| CMISUtils.CMIS_SECONDARY.equalsIgnoreCase(child.getBaseTypeId().toString())) {
			return path;
		}
	...
	public Document getRootDocument(String uri)
	...
	// Get the Content of a CMIS file as a String , which is later converted
	// into an InputStream and set as HCI_content.
	public InputStream getContentStream(String uri)
		...
		if (actions.getAllowableActions().contains(Action.CAN_GET_PROPERTIES)) {

			for (Property<?> prop : document.getProperties()) {
				if (prop.getId().equalsIgnoreCase(PropertyIds.CONTENT_STREAM_LENGTH)) {
					contentLength = prop.getValueAsString();
					break;
				}
			}
			if (contentLength == null || contentLength.equalsIgnoreCase("null")) {
				return new ByteArrayInputStream(CMISUtils.EMPTY_STRING.getBytes());
			}
