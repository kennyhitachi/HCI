...
public class ReutersRawGenerator extends BaseMetadataGenerator {
	...
	protected SimpleDateFormat mOutputChildFolderDateFormat = new SimpleDateFormat("MM-dd'T'HHmmZ");
	 ...
	 */
	private class ReutersRawGeneratorProperties extends BaseGeneratorProperties {
		public ReutersRawGeneratorProperties() {
			super();
		}
	...
	ReutersRawGeneratorProperties mProps = new ReutersRawGeneratorProperties();
	...
	public LinkedList<ObjectContainer> getMetadataList(BaseWorkItem inItem) {
		...
		// To work properly here, we must have an FileSystemItem.  Make sure we do.
		...
		// First the grandparent folder name contains the country code needed for looking up the connection info.
		String folderName = thisItem.getFile().getParentFile().getParentFile().getParentFile().getName();
		String regionCode = folderName.substring(0, Math.max(folderName.lastIndexOf(mProps.getRegionFolderPostFix()), 0));
